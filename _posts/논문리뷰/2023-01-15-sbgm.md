---
title: "[논문리뷰/Diffusion] Score-Based Generative Modeling through Stochastic Differential Equations"
...
tags:
  ...
  - Computer Vision
  - AI
excerpt: "Score-Based Generative Modeling through Stochastic Differential Equations 논문 리뷰"
use_math: true
classes: wide
---

> ICLR 2021. [[Paper](https://arxiv.org/abs/2105.05233)] [[Github](https://github.com/openai/guided-diffusion)]  
...
특히, 저자들은 유한 개의 noise 분포 대신 diffusion process에 의해 시간이 지남에 따라 진화하는 분포의 연속체(continuum)를 고려하였다. 이 프로세스는 점진적으로 데이터 포인트를 random noise로 확산시키고 데이터에 의존하지 않고 학습 가능한 parameter가 없는 SDE에 의해 가능하다. 이 프로세스를 reverse하여 random noise를 샘플 생성을 위한 데이터로 만들 수 있습니다. 결정적으로, 이 reverse process는 reverse-time SDE를 충족하며, 이는 시간의 함수로서 주변 확률 밀도의 score가 주어진 forward SDE에서 유도할 수 있다. 따라서 시간에 의존하는 신경망을 학습시켜 score를 추정한 다음 numerical SDE solver로 샘플을 생성하여 reverse-time SDE를 근사할 수 있다. 
...
여기서 $$\lambda : [0:T] \rightarrow \mathbb{R}_{>0}$$는 양의 가중치 함수이며, $t$는 $[0:T]$에서 uniform하게 샘플링한다. 또한 $x(0) \sim p_0 (x)$이고 $x(t) \sim p_{0t} (x(t) \vert x(0))$이다. 데이터와 모델의 capacity가 충분하다면 score matching은 최적해 $s_{\theta^\ast} (x,t)$를 보장하며, 거의 대부분의 $x$와 $$에 대해 최적해와 $\nabla_x \log p_t (x)$가 같다. 
...
위 식은 score를 아는 경우 SDE에 의해 결정된다. 이 ODE를 **probability flow ODE**라 부른다. 또한 score function이 신경망에 의해 근사되기 때문에 nueral ODE이다. 
