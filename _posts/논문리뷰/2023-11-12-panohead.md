...
이러한 문제를 해결하기 위해 우리는 전경과 배경을 분리하기 위한 배경 generator와 삼중 discriminator, StyleGAN backbone과 여전히 호환되면서 효율적이면서도 표현력이 풍부한 삼중 그리드 표현, 학습 중에 렌더링 카메라를 동적으로 조정하는 자체 적응 모듈을 갖춘 2단계 이미지 정렬 방식을 도입한다. 모델의 전체 파이프라인은 아래 그림에 나와 있다.
