---
title: "[논문리뷰/Diffusion] Diffusion models for Handwriting Generation"
...
$$
\begin{equation}
\alpha_t  = 1 - \beta_t, \quad \overline{\alpha}_t = \prod_{s=1}^t \alpha_s, \quad \epsilon \sim \mathcal{N} (0, I), \quad y_t = \sqrt{\overline{\alpha}_t} y_0 + \sqrt{1 - \overline{\alpha}_t} \epsilon
\end{equation}
$$
...
$$
\begin{equation}
L_{t-1} = \mathbb{E}_{y_0, \epsilon} \bigg( \frac{1}{2\sigma^2} \bigg\| \frac{1}{\sqrt{\alpha}_t} \bigg(y_t - \frac{\beta_t}{\sqrt{1-\overline{\alpha}_t}} \epsilon \bigg) - \mu_\theta (y_t, t) \bigg\|^2 \bigg)
\end{equation}
$$
...
$$
\begin{equation}
L_{t-1} = \mathbb{E}_{t, \epsilon} [C_t \| \epsilon - \epsilon_\theta (y_t, t) \|_2^2], \quad C_t = \frac{\beta_t^2}{2\sigma_t^2 \alpha_t (1- \overline{\alpha}_t)}
\end{equation}
$$
...
$$
\begin{equation}
y_{t-1} = \frac{1}{\sqrt{\alpha_t}} \bigg( y_t - \frac{\beta_t}{\sqrt{1 - \overline{\alpha}_t}} \epsilon_\theta (y_t, t) \bigg) + \sigma_t z, \quad z \sim \mathcal{N} (0, I)
\end{equation}
$$
...
$$
\begin{equation}
y_0 \approx \hat{y}_0 = \frac{1}{\sqrt{\overline{\alpha}_t}} (y_t - \sqrt{1- \overline{\alpha}_t} \epsilon_\theta (y_t, t))
\end{equation}
$$
...
$$
\begin{equation}
y_{t-1} = \sqrt{\overline{\alpha}_{t-1}} \hat{y}_0 + \sqrt{1 - \overline{\alpha}_{t-1}} z, \quad z \sim \mathcal{N} (0, I)
\end{equation}
$$
...
$$
\begin{equation}
y_{t-1} = \frac{1}{\sqrt{\alpha_t}} (y_t - \sqrt{1 - \overline{\alpha}_t} \epsilon_\theta (y_t, t)) + \sqrt{1 - \overline{\alpha}_{t-1}} z, \quad z \sim \mathcal{N} (0, I)
\end{equation}
$$
...
따라서 각 데이터 포인트 $x_0$를 stroke를 나타내는 $y_0$와 stroke가 그려졌는 지를 나타내는 $d_0$로 나누어야 한다. 각 step $t$에서 모델 $$d_\theta (y_t, c, s, \sqrt{\overline{\alpha}_t})$$는 stroke가 그려졌는지 $$\hat{d}_0$$를 추정한다. $d_\theta$는 $\epsilon_\theta$와 모든 파라미터를 공유하며, stroke loss와 pen-draw loss는 다음과 같다. 

$$
\begin{equation}
L_\textrm{stroke} (\theta) = \| \epsilon - \epsilon_\theta (y_t, c, s, \sqrt{\overline{\alpha}_t}) \|_2^2 \\
L_\textrm{drawn} (\theta) = -d_0 \log (\hat{d}_0) - (1-d_0) \log (1-\hat{d}_0)
\end{equation}
$$

Pen-draw를 높은 noise level에서 예측하기 어렵기 때문에 pen-draw loss를 noise level에 따라 가중치를 주는 것이 좋다고 한다. 따라서 위의 pen-draw loss에 가중치 $\overline{\alpha}$를 준다. 

Noise schedule을 어떻게 선택하는 지가 고품질 샘플을 생성하는 데에 중요하다. Wavegrad 논문에서는 이산적인 인덱스 $t$ 대신에 연속적인 noise level $\sqrt{\overline{\alpha}}$을 조건으로 주는 것을 제안하였으며, 재학습 없이 여러 noise schedule을 샘플링에서 사용할 수 있게 된다.

연속적인 noise level에서 신경망을 컨디셔닝하기 위하여 저자들은 먼저 noise schedule $l$을 정의하였다. $l_0 = 1$이고 $$l_t = \sqrt{\overline{\alpha}_t}$$라고 두고 $$t \sim \textrm{Uniform} (\{1,\cdots,T\})$$에 대하여 $$\sqrt{\overline{\alpha}} \sim \textrm{Uniform} (l_{t-1}, l_t)$$로 모델을 컨디셔닝한다. 
...
Noise level $\sqrt{\overline{\alpha}}$는 2개의 FC layer로 이루어진 feedforward network를 통과한 후 인코더와 diffusion model에 조건으로 주어진다. 
...
- 학습
  - $T = 60$, $\beta_t = 0.02 + e^g$ (g는 10^{-5}에서 0.4)
