---
title: "[논문리뷰/Diffusion] DiffSVC: A Diffusion Probabilistic Model for Singing Voice Conversion"
...
DiffSVC는 diffusion model에 기반한 SVC 시스템이다. 본 논문은 ASR acousic model로 노래에서 phonetic posteriorgram (PPG)를 추출한다. Diffusion model은 content, 멜로디, 소리 크기의 feature를 조건으로 받아 Gaussian noise로부터 mel spectrogram을 점진적으로 복구한다. 

DiffSVC는 기존의 state-of-the-art SVC보다 목소리의 자연스러움과 유사성에서 우수한 conversion 성능을 보인다. 본 논문은 다음의 2가지 기여가 있다. 
...
$$
\begin{equation}
q(y_t | y_0) = \mathcal{N} (y_t; \sqrt{\overline{\alpha}_t} y_0, (1-\overline{\alpha}_t) I), \quad \quad \textrm{where} \quad \alpha_t := 1- \beta_t, \; \overline{\alpha}_t = \prod_{s=1}^t \alpha_s
\end{equation}
$$
...
이다. $\mu_\theta$는 noise $\epsilon$에 대응되는 $\epsilon_\theta$로 parameterize되며, $\epsilon_\theta$는 $y_t$, timestep $t$, PPG $x$, Log-F0 $f_0$, loudness $l$이 입력으로 주어진다. 

학습 과정도 DDPM과 동일하다. 학습 시에는 $q(y_t \vert y_0)$ 식을 이용하여 $\epsilon_\theta$에 

$$
\begin{equation}
y_t = \sqrt{\overline{\alpha}_t} y_0 + \sqrt{1-\overline{\alpha}_t}\epsilon
\end{equation}
$$
...
$$
\begin{equation}
\mathbb{E}_{y_0, t} (\| \epsilon - \epsilon_\theta(\sqrt{\overline{\alpha}_t} y_0 + \sqrt{1-\overline{\alpha}_t}\epsilon, t, x, f_0, l)\|_2^2)
\end{equation}
$$

샘플링도 DDPM과 동일하다. $y_T \sim \mathcal{N} (0,I)$에서 시작하여 $t=T$부터 $t=1$까지 reverse transition $p_\theta (y_{t-1} \vert y_t)$ 으로 한 step씩 샘플링하여 이미지가 샘플링된다. 

$$
\begin{equation}
y_{t-1} = \frac{1}{\sqrt{\alpha}_t} (y_t - \frac{1-\alpha_t}{\sqrt{1-\overline{\alpha}_t}} \epsilon_\theta (y_t, t, x, f_0, l)) + \sigma_t z,
\quad \quad \sigma_t^2 = \frac{1 -\overline{\alpha}_{t-1}}{1 - \overline{\alpha}_t} \beta_t
\end{equation}
$$

$t > 1$일 때 $z \sim \mathcal{N} (0, I)$이고 $t = 1$일 때 $z = 0$이다. 최종적으로 $y_0$가 생성 데이터가 된다. 
...
저자들은 content, 멜로디, loudness(소리 크기)를 SVC task의 중요한 요소로 보았다. 먼저 Content feature로 PPG $x$를 추출하기 위하여 Deep-FSMN (DFSMN)-based ASR acousic model을 학습시켰다. ASR model은 cross-entropy loss로 학습되었으며, 중국어 SVC를 위한 음소(phonemes)를 ground truth label로 사용하였다.
...
Log-F0 feature $f_0$와 loudness feature $l$은 같은 방식으로 처리된다. 먼저 256개의 bin으로 양자화한 후 각각 melody embedding lookup table과 loudness embedding lookup table을 통과한다. 

PPG prenet을 통과한 PPG $x$와 embedding lookup table을 통과한 Log-F0 $f_0$와 loudness $l$는 elementwise하게 더해져 conditioner $e$가 된다. $e$는 diffusion decoder의 추가 입력으로 주어지는 token이 된다. 

### 2. Diffusion Modeling
#### Step Encoding
Timestep $t$는 sinusoidal position encoding으로 다음과 같은 128차원 벡터 $t_{emb}$ 변환된다.
...
그런 다음 $t_{emb}$를 2개의 FC layer와 Swish 활성화 함수로 가공된다. 
...
먼저 mel spectrogram을 Conv1x1 layer와 ReLU를 통과시킨다. 그런 다음 step encoder의 output을 더하고 $N$개의 residual block에 입력으로 넣는다. Conditioner $e$는 각 residual block의 Conv1x1 layer에 입력으로 넣어준 후 그 output을 mel spectrogram feature와 step encoder feature를 더한 값에 더해준다.

그런 다음 feature map을 Wavenet 논문에서 소개된 gated mechanism를 적용한다. 그 결과로 각 residual block은 skip output 1개와 다음 residual block에 입력으로 사용할 output 1개가 나온다. $N$개의 skip output을 모두 더한 다음 Conv1x1 layer, ReLU, Conv1x1 layer를 순서대로 통과시키면 noise가 예측된다. 
...
- 전체 timestep $T = 100$. $\beta$는 0.0001에서 0.06까지 선형 간격으로 설정
