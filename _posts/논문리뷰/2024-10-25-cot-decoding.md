...
구체적으로, 표준 질문-답변(QA) 형식인 "Q:[question]\nA:"를 입력을 사용한다. 대부분의 기존 연구에서는 LLM이 이러한 직접 QA 시나리오에서 실패한다고 하였지만, 저자들은 LLM이 greedy decoding된 경로에만 의존할 때 추론에 어려움을 겪는다는 것을 관찰했다. 그러나 top-k 토큰 중에서 대체 경로를 고려할 때 CoT 추론 패턴은 LLM의 디코딩 궤적 내에서 자연스럽게 나타난다. 
