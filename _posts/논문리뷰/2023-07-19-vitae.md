...
DeiT는 학습을 용이하게 하고 성능을 향상시키기 위해 CNN에서 transformer로 지식을 distill하는 아이디어를 탐구하였다. 그러나 teacher로 CNN 모델이 필요하고 추가 학습 비용이 필요하다.
...
$$
\begin{equation}
f_i^\textrm{ms} \in \mathbb{R}^{(W_i / p)×(H_i / p)×( \vert \mathcal{S}_i \vert D)}
\end{equation}
$$
