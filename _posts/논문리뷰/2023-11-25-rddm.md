...
여기서 hyperparameter $$\lambda_\textrm{res}, \lambda_\epsilon \sim \{0, 1\}$$와 학습 입력 이미지는 $I_0$, $I_\textrm{res}$, $\epsilon$을 사용하여 합성된다. 실제로 $I_0$를 $I_0 = I_\textrm{in} − I_\textrm{res}$로 대체할 수 있다. 
